...
def test_sample_schema_is_explicitly_cross_sectional_and_needs_no_event_dates(
    ...
    assert audit.data_construct == "cross_sectional_status_only"
    assert audit.event_date_columns_present == ()
    assert not audit.historical_snapshots_available
    assert frame.attrs["data_construct"] == "cross_sectional_status_only"
...
def test_event_and_snapshot_aliases_are_preserved_parsed_and_audited(
    ...
    assert audit.data_construct == (
        "cross_sectional_status_with_event_dates_and_snapshot_date"
    )
    assert audit.event_date_columns_present == (
        "Satisfaction Date",
        "Cancellation Date",
...
def test_unrecognised_outcome_or_history_header_fails_closed(tmp_path: Path) -> None:
    row = _row()
    row["Payment Confirmed At"] = "01/03/2024"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "does not match observation date",
        ),
    ],
)
