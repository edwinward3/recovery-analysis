"""Orchestrate one offline RT analysis or validate its completed match review.

Inputs are confidential RT/Companies House files plus fixed local settings.
Row-level matches, sample pairs, partitions and models are written only below
``rt_internal``. Only explicitly allowlisted aggregate reports are copied to an
egress candidate directory after disclosure checks. No network or shell calls
...
from .models import ModelEvaluation, fit_evaluate_models, prepare_model_cohort, write_model_artifacts
...
def analyze(
    *,
    ...
    """Execute one fresh, cache-free diagnostic or locked analysis."""
    ...
    with recorder.stage("E3_prepare_cohort") as record:
        ...
        ].to_csv(paths.internal / SPLIT_FILENAME, index=False, encoding="utf-8-sig")
    ...
    with recorder.stage("E1_to_E4_aggregate_reports"):
        audit_counts = build_data_audit_counts(judgments, audit)
        funnel = pd.DataFrame(
            [
                {"stage": name, "rows": int(rows)}
                for name, rows in cohort.funnel.items()
            ]
        )
        ...
        summary_context = _summary_context(
            stage, audit, judgments, matches, evaluation, PAIR_FILENAME
        )
    ...
    write_e5(aggregate, recorder, manifest)
    ...
    with recorder.stage("E5_disclosure_gate") as record:
        ...
        record["staged_files"] = len(disclosure.staged_files)
        record["suppressed_rows"] = sum(
            count for _, count in disclosure.suppressed_rows
        )
    manifest["disclosure"].update(
        {
            "status": "pass",
            "staged_files": list(disclosure.staged_files),
            "suppressed_rows": [
                {"file": filename, "rows": count}
                for filename, count in disclosure.suppressed_rows
            ],
        }
    )
    # E5 is finalized after the gate, then the complete egress directory is
    # scanned again so the receipt cannot bypass disclosure validation.
    write_e5(aggregate, recorder, manifest)
    write_e5(paths.egress, recorder, manifest)
    validate_egress(paths.egress, known_identifiers=known_identifiers)
...
def review_completed_sample(
    *,
    ...
    reasons = list(model_only.get("reasons", []))
    if state.get("stage") != "locked":
        reasons.append("diagnostic_run_cannot_receive_final_status")
    if not review.gate_passed:
        reasons.extend(f"match_review: {reason}" for reason in review.gate_reasons)
    combined_passed = not reasons and bool(model_only.get("passed"))
    if not model_only.get("passed") and not model_only.get("reasons"):
    ...
    with TemporaryDirectory(prefix="review-aggregate-") as temporary:
        ...
        decision_digest = review_decision_digest(frame)
        combined = {
            "schema_version": 1,
            ...
            # A digest of a nearly all-correct decision vector is itself
            # low-entropy and can be brute-forced. Do not publish that exact
            # outcome binding when the underlying small cell is suppressed.
            "review_decision_fingerprint": (
                None if auto_detail_suppressed else decision_digest
            ),
        ...
        stage_egress(
            ...
            allowlist={
                ...
                "FINAL_STATUS.json": None,
                "FINAL_STATUS.txt": None,
            },
            min_cell_n=settings.min_cell_n,
        )
...
def review_decision_digest(frame: pd.DataFrame) -> str:
    """Hash completed decisions and notes without exposing reviewed rows."""

    required = {"review_row_id", "review_decision", "review_notes"}
    missing = required - set(frame.columns)
    if missing:
        raise RunFailure(f"review completion is missing column(s): {sorted(missing)}")
    digest = hashlib.sha256()
    rows = frame.sort_values("review_row_id", kind="stable")
    for row in rows.loc[
        :, ["review_row_id", "review_decision", "review_notes"]
    ].itertuples(index=False, name=None):
        identifier = unicodedata.normalize("NFKC", str(row[0])).strip()
        decision = unicodedata.normalize("NFKC", str(row[1])).strip().casefold()
        notes = unicodedata.normalize("NFKC", str(row[2])).strip()
        digest.update(f"{identifier}\x1f{decision}\x1f{notes}\x1e".encode("utf-8"))
    return digest.hexdigest()
...
def _load_and_validate_review_state(
    ...
    """Bind one completed review to the exact disclosed, locked analysis."""
    ...
    if state.get("immutable_columns") != immutable:
        raise RunFailure("review sample columns differ from the locked schema")
    ...
    if state.get("sample_allocation") != actual_allocation:
        raise RunFailure("review sample allocation differs from the locked state")
    if "sample_seed" not in frame or not frame["sample_seed"].astype(str).eq(
        str(expected_seed)
    ).all():
        raise RunFailure("review sample seed column differs from the locked state")
    ...
    if "sampling_design" not in frame or not frame["sampling_design"].eq(
        ...
        raise RunFailure("review sample design column differs from the locked state")
    ...
    if not isinstance(reasons, list) or not all(
        isinstance(reason, str) and reason in _MODEL_REASON_CODES for reason in reasons
    ):
    ...
    for filename, expected in model_hashes.items():
        ...
        if not artifact.is_file() or _small_file_sha256(artifact) != expected:
            raise RunFailure(f"saved model artefact differs or is missing: {filename}")
    ...
    *,
    ...
    return {
        ...
        "artifact_manifest": {
            ...
            "rt_internal_models_require_separate_rt_approval": True,
        },
        "disclosure": {
            "status": "pending",
...
def _bounded_known_identifiers(sample: pd.DataFrame) -> tuple[str, ...]:
    ...
    for column in columns:
        if column in sample:
            values.extend(sample[column].head(100).astype("string").fillna("").tolist())
    distinctive = (
        value.strip()
        for value in values
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Offline Registry Trust recovery analysis")
    commands = parser.add_subparsers(dest="command", required=True)
    analyze_parser = commands.add_parser("analyze", help="run diagnostic or locked analysis")
    analyze_parser.add_argument("--stage", choices=("diagnostic", "locked"), required=True)
    ...
    review_parser = commands.add_parser("review", help="validate a completed RT match review")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"COMBINED FINAL STATUS: {'PASS' if outcome.combined_passed else 'FAIL'}")
