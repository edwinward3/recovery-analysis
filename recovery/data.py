...
OPTIONAL_RT_COLUMNS: dict[str, object] = {
    ...
    "Defendant Address": "",
    # These fields are not present in the supplied RT sample, but must survive
    # ingestion if RT includes them in a later extract.  Missing optional dates
    # are represented as NaT after parsing.
...
# An unfamiliar field about the outcome or its timing must never be silently
# discarded.  Ordinary ancillary fields remain available in the internal audit
# but do not enter matching or modelling.
...
def _validate_optional_timing(
    ...
    """Reject contradictions while allowing event fields to be wholly absent."""
    ...
    if "Snapshot Date" in present_optional:
        ...
        if snapshot != observed:
            raise ValueError(
                ...
                f"observation date {observed.date().isoformat()}"
            )
...
def _classify_data_construct(present_optional: set[str]) -> str:
    ...
    if not event_columns and not snapshot_present:
        return "cross_sectional_status_only"
    if event_columns and snapshot_present:
        return "cross_sectional_status_with_event_dates_and_snapshot_date"
    if event_columns:
        return "cross_sectional_status_with_event_dates"
    return "cross_sectional_status_with_snapshot_date"
...
def read_rt_extract(
    ...
    audit = DataAudit(
        ...
        # One row per unique judgment can be a dated snapshot, but not snapshot
        # history.  Repeated-ID snapshot input is rejected above for redesign.
